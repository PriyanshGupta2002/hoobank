  ...
  return (
    <section className={`${styles.flexCenter} flex-col ${styles.paddingY}`}>
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}> 
        ...
        <div className='flex-[1.5] w-full justify-between flex-row flex flex-wrap md:mt-0 mt-10'>
            ...
            <div className={`flex flex-col ss:my-0 my-4 min-w-[150px]`} key={footerlink.key}>
              ...
              <ul className='list-none mt-4'>
                {footerlink.links.map((link,idx)=>(
                  <li key={idx} className={`font-poppins font-normal text-[16px] ${idx!==footerlink.length-1?'mb-4':'mb-0'} leading-[24px] text-dimWhite hover:text-secondary cursor-pointer`}>
                      {link.name}
                  </li>
                ))}
              </ul>
      ...
      <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]'>
            ...
            <div className='flex flex-row md:mt-0 mt-6'>
                    {socialMedia.map((social,idx)=>(
                      ...
                      src={social.icon}
                      alt={social.id}
                      className={`w-[21px] h-[21px] object-contain cursor-pointer ${idx!==socialMedia.length-1?'mr-6':'mr-0'}`}
                      />
                    ))}
            </div>
      </div>
